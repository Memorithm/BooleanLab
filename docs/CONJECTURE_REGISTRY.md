...
### BL-C013 — Hybrid-induced Boolean-function discovery

**X:** open mathematical-domain family `X`  
**Coupling:** any declared Boolean × X coupling followed by exact Boolean projection  
**Hypothesis:** some Boolean × X systems induce Boolean functions whose construction, equivalence class, or measured property trade-off is not recovered by the matched Boolean-only search grammar under the same search budget.  
**Null hypothesis:** every induced function is either already generated by the matched Boolean-only grammar, equivalent to a known generated function under the declared equivalence relation, or provides no new property trade-off.  
**Primary objects:** scalar Boolean functions `F_2^n -> F_2` and, in a separate track, vectorial Boolean functions `F_2^n -> F_2^m`.  
**Required novelty gates:** exact truth table where feasible, algebraic normal form, Walsh spectrum, algebraic degree, nonlinearity, balancedness, correlation immunity/resiliency where applicable, structural/circuit cost, canonical fingerprints, and equivalence screening appropriate to the object class. Vectorial candidates require separate differential and equivalence analysis.  
**Falsification:** after canonicalization and matched search, no hybrid-induced candidate provides a new construction rule, inequivalent family, or non-dominated property/cost point.  
**Non-claim:** an unseen truth table is not by itself a mathematical discovery.
