#!/usr/bin/env bash
# Preserve a complete numerical-development run, including resolved dependencies.
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT"
if (( $# > 1 )); then
  echo "usage: bash scripts/run-bl14-structured-relu.sh [new-output-directory]" >&2
  exit 2
fi
...
OUT="${1:-target/bl14-structured-evidence}"
if [[ -e "$OUT" ]]; then
...
sha256sum Cargo.toml crates/booleanlab-discovery/Cargo.toml \
  ...
  experiments/BL-14.2.2-STRUCTURED-RELU-PROTOCOL.md \
  scripts/run-bl14-structured-relu.sh > "$OUT/source-files.sha256"
cargo test --locked -p booleanlab-discovery --bin bl14_structured_relu \
  2>&1 | tee "$OUT/tests.txt"
cargo build --locked --release -p booleanlab-discovery --bin bl14_structured_relu \
  2>&1 | tee "$OUT/build.txt"
target/release/bl14_structured_relu | tee "$OUT/report.tsv"
target/release/bl14_structured_relu > "$OUT/replay.tsv"
