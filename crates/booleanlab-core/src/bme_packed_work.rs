//! Shape-derived representation-work accounting for packed Boolean Matrix Equation baselines.
//!
...
pub struct PackedBmeWorkEstimate {
    ...
    /// Maximum packed-word evaluations if every cell consumes every packed word.
    ///
    /// Individual equations may short-circuit. In particular OR-AND can stop after the first
    /// intersecting word, so this is a shape-derived upper bound rather than a measured count.
    pub max_packed_word_evaluations: u128,
...
/// Returns shape-derived work and exact storage counters for the row/column packed baseline.
///
...
///
/// `max_packed_word_evaluations` is an upper bound assuming every output cell visits every packed
/// word. Data-dependent short-circuiting can make the actual executed count smaller.
/// `input_payload_bytes` counts exactly the declared packed words for the left-row and right-column
...
/// [`PackedBmeWorkError::ArithmeticOverflow`] if a counter cannot be represented in `u128`.
pub fn packed_bme_work_estimate(
    ...
    let logical_bit_pairs = output_cells
        ...
        .ok_or(PackedBmeWorkError::ArithmeticOverflow)?;
    let max_packed_word_evaluations = output_cells
...
mod tests {
    ...
    #[test]
    fn counts_word_boundaries_without_turning_bounds_into_measurements() {
        ...
        assert_eq!(at_64.max_packed_word_evaluations, 6);
        ...
        assert_eq!(at_65.max_packed_word_evaluations, 12);
