...
- `bl14.matched-observation-set.v1` now binds exactly one observation to every frozen arm in canonical order and requires the stable fingerprint of the complete exact arm/mask set. It requires one independently declared quality metric and one shared resource-evidence kind/protocol/provenance, preserves traffic as unavailable when it was not observed, forbids latency fields on reference-model accounting, and requires latency for measured hardware evidence. It performs no ranking, selection or promotion and therefore cannot turn the observation envelope into a sparsity-benefit claim.
